...
describe('Linked list', () => {
  ...
  it('Can List.append multiple items and List.get them', () => {
    ...
    for (let i = 0; i < nums.length; ++i) {
      expect(get(i, list)).to.be.undefined();
      List.append(nums[i], list);
      expect(get(i, list)).to.equal(nums[i]);
    }
  });
  ...
  it('Can List.removeAt items from the beginning', () => {
    ...
    expect(Lists.get(0, list)).to.equal(2);
  });
  ...
  it('Can List.removeAt items from the end', () => {
    ...
    expect(List.get(3, list)).to.equal(3);
  ...
  it('Can use List.insert to insert stuff', () => {
    ...
    expect(List.get(List.length(list) - 1, list)).to.equal('to6o', 'insert at end');
  });
  ...
  it('Can use List.remove to remove items', () => {
    ...
    List.remove(List.length(list) - 1, list);
